...
logger.log([
  ...
  {
    message: `This Bot Is By ZINO and MOHAMED`,
    color: "white",
  },
]);
...
global.LUNA = require("./includes/LUNA/ults.js")
...
try {
  var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "LUNAstate.json"));
...
const DB_by_allou = await require('./DB/controller/index.js')(loginApiData);
    ...
    global.client.timeStart = new Date().getTime(),
      function() {
        const listCommand = readdirSync(global.client.mainPath + '/LUNAScripts/commands').filter(command => command.endsWith('.js') && !command.includes('example') && !global.config.commandDisabled.includes(command));
        for (const command of listCommand) {
          try {
            var module = require(global.client.mainPath + '/LUNAScripts/commands/' + command);
      ...
      function() {
        const events = readdirSync(global.client.mainPath + '/LUNAScripts/events').filter(event => event.endsWith('.js') && !global.config.eventDisabled.includes(event));
        for (const ev of events) {
          try {
            var event = require(global.client.mainPath + '/LUNAScripts/events/' + ev);
    ...
    console.log(redToGreen("━".repeat(43), { interpolation: "hsv" }));
    console.log(cv(`\n` + `●──LOADING LOADER──●`));
    logger.log([{
      message: "[ LOADER ]: ",
      color: ["red", "cyan"],
